...
// Something
const char* serverURL = "http://172.20.10.7:5000/arduino";
...
void sendData(float data) {  // This function will send the measured data to the web server using HTTPClient
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String httpRequestData = "{\"2221\":" + String(data, 2) + "}";     // This is json format. It has some name(idk what), and a decibel value. Decibel value should actively change in the loop.
...
void loop() {
  ...
  float data = (int16_t)(i2sReadrawBuff[0]|i2sReadrawBuff[1]<<8);
